"""
AI Insight Hub — Phase 4 Decision Tree Service
Handles Decision Tree model artifact access, applicant input validation, and
...
Responsibilities:
  ...
  - Validate incoming applicant input against the ranges recorded at training time
  - Execute r_models/decision_tree/predict.R via RRunner (never retrains)
  - Read the real metrics / tree-structure artifacts produced by train.R
"""
import json
import logging
import os
...
logger = logging.getLogger("ai_insight_hub.decision_tree")

# Model identity — the single source of truth for the Phase 4 model
MODEL_NAME = "Decision Tree"
PREDICT_SCRIPT = "r_models/decision_tree/predict.R"
TRAIN_SCRIPT = "r_models/decision_tree/train.R"

# Canonical feature order, matching the formula fitted in train.R
FEATURES: Tuple[str, ...] = ("age", "income", "credit_score", "existing_loans", "employment_years")

# The three risk tiers the model is trained to predict. These must match the
# class factor levels set in train.R; an unexpected class in a response is
# rejected rather than passed through to the user.
...
FIELD_RULES: Dict[str, Dict[str, Any]] = {
    "age": {"min": 0, "exclusive_min": True, "label": "Age"},
    "income": {"min": 0, "exclusive_min": True, "label": "Annual Income"},
    "credit_score": {"min": 300, "max": 900, "label": "Credit Score"},
    "existing_loans": {"min": 0, "exclusive_min": False, "label": "Existing Loans"},
    "employment_years": {"min": 0, "exclusive_min": False, "label": "Employment Years"},
}


def get_decision_tree_dir() -> str:
    """Returns the absolute path to the r_models/decision_tree directory."""
    base_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
    return os.path.join(base_dir, "r_models", "decision_tree")


def get_model_path() -> str:
    """Absolute path to the trained model bundle (model.rds)."""
    return os.path.join(get_decision_tree_dir(), "model.rds")


def get_metrics_path() -> str:
    """Absolute path to the training metrics artifact (metrics.json)."""
    return os.path.join(get_decision_tree_dir(), "metrics.json")


def get_tree_path() -> str:
    """Absolute path to the extracted tree-structure artifact (tree.json)."""
    return os.path.join(get_decision_tree_dir(), "tree.json")
...
def validate_applicant_input(payload: Any) -> Tuple[Dict[str, float], List[str]]:
    """
    Validates an applicant feature payload.

    Checks, in order of usefulness to the caller:
      - payload is a JSON object
      - every required feature is present
      - every value is a finite number
      - age / income > 0, counts and years >= 0
      - credit_score within 300-900
      - no unrecognised fields

    Values outside the dataset's observed range are NOT rejected — the model
    is still asked to classify them, and the caller reports them as a warning
    via `find_extrapolation_warnings` so the user can see the prediction is
    ...
    if not isinstance(payload, dict):
        return cleaned, ["Request body must be a JSON object containing the applicant features."]
    ...
    # Reject unexpected keys so a typo like "creditscore" is not silently ignored.
...
def find_extrapolation_warnings(cleaned: Dict[str, float]) -> List[Dict[str, Any]]:
    ...
    Such a value is still answered by the tree — a CART tree always routes a
    record down some branch — but the answer is an extrapolation rather than an
    interpolation, so the caller is told which inputs to be careful about.
...
def predict_risk(validated_input: Dict[str, float]) -> Dict[str, Any]:
    """
    ...
    if not is_model_trained():
        return {
            ...
            "error": (
                ...
                "Run 'Rscript r_models/decision_tree/train.R' to generate model.rds."
            ),
            "status_code": 503,
        }
    ...
    if not isinstance(prediction, str) or prediction not in CLASSES:
        logger.error(f"R returned an invalid risk class: {prediction!r}")
        return {
            "success": False,
            "model": MODEL_NAME,
            "error": f"The R engine returned an unexpected risk class: {prediction!r}",
            "status_code": 502,
        }
    ...
    response: Dict[str, Any] = {
        ...
        "decision_depth": data.get("decision_depth"),
    ...
    # Confidence is only surfaced when R actually calculated one.
    ...
    if not metrics:
        return {
            ...
            "error": (
                ...
                "Run 'Rscript r_models/decision_tree/train.R' to train the model."
            ),
            "status_code": 503,
        }
...
def get_tree_structure() -> Dict[str, Any]:
    """
    Returns the REAL fitted decision-tree structure written by train.R.

    The payload is the tree that rpart grew, extracted node by node and
    verified by train.R to route records identically to rpart's own predict().
    It is not a hand-drawn or decorative tree.
    """
    tree = _read_json(get_tree_path())
    if not tree:
        return {
            "success": False,
            "model": MODEL_NAME,
            "error": (
                "Tree structure is not available. "
                "Run 'Rscript r_models/decision_tree/train.R' to train the model."
            ),
            "status_code": 503,
        }
    ...
    if not isinstance(root, dict) or "id" not in root:
        return {
            "success": False,
            "model": MODEL_NAME,
            "error": "The tree artifact is malformed — it has no root node.",
            "status_code": 500,
        }

    return {
        "success": True,
        "model": tree.get("model", MODEL_NAME),
        "algorithm": tree.get("algorithm"),
        "target": tree.get("target", "risk"),
        "classes": tree.get("classes", list(CLASSES)),
        "features": tree.get("features", list(FEATURES)),
        "feature_labels": tree.get("feature_labels", {}),
        "trained_at": tree.get("trained_at"),
        "total_nodes": tree.get("total_nodes"),
        "internal_nodes": tree.get("internal_nodes"),
        "leaf_nodes": tree.get("leaf_nodes"),
        "control": tree.get("control", {}),
        "variable_importance": tree.get("variable_importance", []),
        "root": root,
        "status_code": 200,
    }
